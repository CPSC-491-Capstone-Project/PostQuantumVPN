...
using namespace core::cryptography::ml_kem;
...
bool MlKemTest_Roundtrip_768() {
    ...
    auto enc = Encapsulate(kp->public_key);
    if (!enc) return test_helper("encapsulate", "nullopt");

    auto ss = Decapsulate(*kp, enc->ciphertext);
    if (!ss) return test_helper("decapsulate", "nullopt");

    return test_helper("match", BytesMatch(enc->shared_secret, *ss));
}
...
bool MlKemTest_EncapsulateUniqueness() {
    ...
    auto enc1 = Encapsulate(kp->public_key);
    auto enc2 = Encapsulate(kp->public_key);
    if (!enc1 || !enc2) return test_helper("encapsulate", "nullopt");

    return test_helper("mismatch", BytesMatch(enc1->shared_secret, enc2->shared_secret));
}
...
bool MlKemTest_WrongKeyImplicitRejection() {
    auto kp_alice = GenerateKeyPair();
    auto kp_bob   = GenerateKeyPair();
    if (!kp_alice || !kp_bob) return test_helper("keygen", "nullopt");

    auto enc = Encapsulate(kp_alice->public_key);
    if (!enc) return test_helper("encapsulate", "nullopt");

    auto ss = Decapsulate(*kp_bob, enc->ciphertext);
    if (!ss) return test_helper("decapsulate", "nullopt");

    return test_helper("mismatch", BytesMatch(enc->shared_secret, *ss));
}