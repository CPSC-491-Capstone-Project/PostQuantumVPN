...
using namespace core::cryptography::x25519;
...
bool X25519Test_GenerateKeyPair_PrivateKeySize() {
    ...
    return test_helper(
        std::to_string(kPrivateKeyBytes),
...
bool X25519Test_PublicKeyFromPrivate_MatchesKeyPair() {
    ...
    auto derived_pub = PublicKeyFromPrivate(kp->private_key);
...
bool X25519Test_PublicKeyFromPrivate_Deterministic() {
    ...
    auto pub1 = PublicKeyFromPrivate(kp->private_key);
    auto pub2 = PublicKeyFromPrivate(kp->private_key);
...
bool X25519Test_PublicKeyFromPrivate_UniquePerPrivateKey() {
    ...
    auto pub1 = PublicKeyFromPrivate(kp1->private_key);
    auto pub2 = PublicKeyFromPrivate(kp2->private_key);
    if (!pub1 || !pub2) return test_helper("derive", "nullopt");
...
bool X25519Test_DeriveSharedSecret_Succeeds() {
    auto kp_a = GenerateKeyPair();
    auto kp_b = GenerateKeyPair();
    if (!kp_a || !kp_b) return test_helper("setup", "nullopt");

    auto secret = DeriveSharedSecret(kp_a->private_key, kp_b->public_key);
...
bool X25519Test_DeriveSharedSecret_Size() {
    ...
    return test_helper(
        std::to_string(kSharedSecretBytes),
...
bool X25519Test_DeriveSharedSecret_Commutative() {
    ...
    auto secret_ab = DeriveSharedSecret(kp_a->private_key, kp_b->public_key);
    auto secret_ba = DeriveSharedSecret(kp_b->private_key, kp_a->public_key);
...
bool X25519Test_DeriveSharedSecret_Deterministic() {
    ...
    auto s1 = DeriveSharedSecret(kp_a->private_key, kp_b->public_key);
    auto s2 = DeriveSharedSecret(kp_a->private_key, kp_b->public_key);
...
bool X25519Test_DeriveSharedSecret_DifferentPeerGivesDifferentSecret() {
    auto kp_a  = GenerateKeyPair();
    auto kp_b1 = GenerateKeyPair();
    auto kp_b2 = GenerateKeyPair();
    if (!kp_a || !kp_b1 || !kp_b2) return test_helper("setup", "nullopt");

    auto s1 = DeriveSharedSecret(kp_a->private_key, kp_b1->public_key);
    auto s2 = DeriveSharedSecret(kp_a->private_key, kp_b2->public_key);
    if (!s1 || !s2) return test_helper("derive", "nullopt");
...
bool X25519Test_DeriveSharedSecret_WrongPrivateKey() {
    auto kp_a1 = GenerateKeyPair();
    auto kp_a2 = GenerateKeyPair(); // impostor
    auto kp_b  = GenerateKeyPair();
    if (!kp_a1 || !kp_a2 || !kp_b) return test_helper("setup", "nullopt");

    auto s_real   = DeriveSharedSecret(kp_a1->private_key, kp_b->public_key);
    auto s_impostor = DeriveSharedSecret(kp_a2->private_key, kp_b->public_key);
...
bool X25519Test_DeriveSharedSecret_ThreePartyIndependent() {
    auto kp_a = GenerateKeyPair();
    auto kp_b = GenerateKeyPair();
    auto kp_c = GenerateKeyPair();
    if (!kp_a || !kp_b || !kp_c) return test_helper("setup", "nullopt");

    auto s_ab = DeriveSharedSecret(kp_a->private_key, kp_b->public_key);
    auto s_ac = DeriveSharedSecret(kp_a->private_key, kp_c->public_key);
    auto s_bc = DeriveSharedSecret(kp_b->private_key, kp_c->public_key);
