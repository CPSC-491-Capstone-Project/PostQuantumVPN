...
ifeq ($(PLATFORM),macos)
	...
	ifdef OPENSSL_PREFIX
		LIB_CFLAGS := -I$(OPENSSL_PREFIX)/include -I/opt/homebrew/include 
		LDFLAGS := -L$(OPENSSL_PREFIX)/lib -lssl -lcrypto -L/opt/homebrew/lib -lblake3
	else
		OPENSSL_FOUND := no
	endif
...
# ----- Source & Dependencies -----
CORE_SRCS   := $(wildcard core/*.$(CXX_EXT)) $(wildcard core/os/$(PLATFORM)/*.$(CXX_EXT)) $(wildcard core/cryptography/*.$(CXX_EXT)) $(wildcard core/utils/*.$(CXX_EXT)) $(wildcard core/network/*.$(CXX_EXT))
